...
@section('title', 'NPDES Actions Report - CDIS Reports')
@section('heading')
    <h3>{{ucfirst(session('county'))}} County Conservation District NPDES Actions
        {{sqlToHtmlDate($request->from)}} to {{sqlToHtmlDate($request->to)}}</h3>
@endsection

@section('content')@php
        $table = '<h2 style="line-height: 2rem;">' . ucwords(session('county')) . ' County Conservation District<br />NPDES Actions<br>';
        ...
        $table .= "<tr><td >" . sqlToHtmlDate($d->np_date_i) . "</td><td >" . clean($d->name) . "<br />" . clean($d->loc) . "<br />" . clean($d->munic) . "<br />" . clean($d->npdes_numb) . "</td><td >" . $d->total_acres . "<br />" . $d->acres_tbd . "</td><td >" .clean( $d->applic) . "<br />" .clean( $d->app_company) . "<br />" . clean($d->app_add1). "<br />" . clean($d->app_add2) . "<br />" . clean($d->app_city ). "," .clean( $d->app_state) . " " . clean($d->app_zip ). "</td><td >" . clean($d->stream ). "<br />" . clean($d->stream_class). "</td><td >" . clean($ownership) . "</td></tr>";
        }
        $table .= '</tbody></table>';
