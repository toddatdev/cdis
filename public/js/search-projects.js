        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (!response.error) {
                    ...
                    //updates the total records
                    $('#found-records').text(response.total);
...
function fillHtmlTemplate(htmlTemplate, data) {
    ...
    $.each(data, function (index, value) {
        ...
        reviewer = (value.project_details && value.project_details.reviewer && value.project_details.reviewer.name != null) ? value.project_details.reviewer.name : '';
        ...
        htmlTemplate.find('.project-technician').text(reviewer);
        htmlTemplate.find('.project-location').text(location);
        htmlTemplate.find('.project-is-closed').html(value.is_closed === 1 ? '<strong>Closed</strong>' : '<strong>Open</strong>');
        if (value.is_closed === 1){
            htmlTemplate[0].style.backgroundColor  = '#eee';
        } else {
            htmlTemplate[0].style.backgroundColor  = '#fff';
        }
